class LotteryController < ApplicationController
    ...
    def refresh
        ...
        all_no = Array.new
        data.each {|row| all_no.concat([ row['no1'],row['no2'],row['no3'],row['no4'],row['no5'],row['no6'] ]) }

        require "rinruby"
        R.assign "all_no", all_no
        R.eval """
            x <- c( all_no )
        ...
        respond_to do |format|
            #format.any { render text: render all_no.to_json }
            format.json { render json: rc.to_json }
        end
    end
